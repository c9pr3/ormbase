...
class ContactContainer extends aContainer {
	...
	/**
	 * Create new aContainer
	 *
	 * @param $name
	 * @return \wplibs\dbinterface\ContactContainer
	 */
	...
	public function createNewContact( $alias, $password, $gender, $phone, $imei ) {
		...
		#
		$sql = $this->getDatabase()->create( 'ALGORITHM=MERGE' )->view( 'v_contact_media_' . $obj->getValue( 'id' ) )->select( new FieldSelection() )->from( 'media' )->where( 'F_contact_id', '=', $obj->getValue( 'id' ) );
		...
		#
		$sql = $this->getDatabase()->create( 'ALGORITHM=MERGE' )->view( 'v_contact_list_' . $obj->getValue( 'id' ) )->select( new FieldSelection( 'c.*', 'cm.member_local_alias', 'cm.contact_member_status', 'm.media_type', 'm.media_visibility', 'm.media_name', 'm.media_path', 'g.geo_time', 'g.geo_accuracy', 'X(g.geo_point) AS geo_latitude', 'Y(g.geo_point) AS geo_longitude' ) )->from( 'contact_has_contacts', 'cm' )->table( 'contact', 'c', 'ON c.id=cm.F_contact_member_id' )->table( 'media', 'm', 'ON m.id=c.F_contact_media_id' )->table( 'geo_location', 'g', 'ON g.id=c.F_contact_geo_location_id' )->where( 'cm.F_contact_owner_id', '=', $obj->getValue( 'id' ) );
		$this->getDatabase()->query( $sql->getQuery()->unparameterize() );

		#
		# create view for contact conversations
		#
		$sql = $this->getDatabase()->create( 'ALGORITHM=MERGE' )->view( 'v_contact_conversation_' . $obj->getValue( 'id' ) )->select( new FieldSelection( 'cv.*', 'cvl.conversation_state', 'm.media_type', 'm.media_visibility', 'm.media_name', 'm.media_path' ) )->from( 'conversation', 'cv' )->table( 'conversation_listener', 'cvl', 'ON cvl.F_conversation_id=cv.id' )->table( 'media', 'm', 'ON m.id=cv.F_conversation_media_id' )->where( 'cvl.F_contact_id', '=', $obj->getValue( 'id' ) );
		...
		#
		$activationCode = password_hash( $obj->getValue( 'contact_alias' ) . $obj->getValue( 'contact_password' ), PASSWORD_BCRYPT );
	 ...
	 *
	 * @param                                $id
	 * @param \wplibs\database\FieldSelection $selector
	 * @return Contact
	 */
	...
	public function getContactsByIMEI( $imei, FieldSelection $selector = null ) {
		...
		$sql = $this->getDatabase()->select( $selector )->from( self::TABLE_NAME )->where( 'contact_imei', '=', (string)$imei );
		$params = $sql->getQueryParams();
	 ...
	 * @param                                $phone
	 * @param \wplibs\database\FieldSelection $selector
	 * @return \wplibs\dbinterface\Contact
	 */
	public function getContactByPhone( $phone, FieldSelection $selector = null ) {

		$sql = $this->getDatabase()->select( $selector )->from( self::TABLE_NAME )->where( 'contact_phone', '=', $phone )->limit( 1 );
		$params = $sql->getQueryParams();
	...
	/**
	 * Get contacts by geo location IDs
	 *
	...
	public function getContactsByGeoLocations( array $geoLocations, FieldSelection $selector = null ) {
		...
		$sql = $this->getDatabase()->select( $selector )->from( self::TABLE_NAME )->where( 'F_contact_geo_location_id', 'IN', '(' . implode( ',', $geoLocationIDs ) . ')' );
		$params = $sql->getQueryParams();
