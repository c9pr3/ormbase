...
abstract class aObject extends DBResultRow {
	...
	final public function getValue( $key ) {
		...
		catch ( \wplibs\exception\DatabaseException $ex ) {
			throw new \wplibs\exception\ObjectException( "Could not find key '$key' in actual resultSet for '" . get_class( $this ) . "' " . var_export( $this->row, true ) );
		}
	...
	final public function setValue( $key, $value, $ignoreMissing = false ) {
		try {
			...
			throw new \wplibs\exception\ObjectException( "Could not find key '$key' in actual resultSet for '" . get_class( $this ) . "'" );
		}
	}
	...
	public function store( $forceOverwritePrimaryKeys = false ) {
		try {
			...
			throw new \wplibs\exception\ObjectException( "Could not store '" . preg_replace( '/^.*\\\\(.*)$/', '\1', get_class( $this ) ) . "' -> " . $ex->getMessage() );
		}
	...
	public function delete() {
		try {
			...
			throw new \wplibs\exception\ObjectException( "Could not delete '" . get_class( $this ) . "' -> " . $ex->getMessage() );
		}
	}
	...
	/**
	 * Get current classname without namespace
	 *
	 * @return string
	 */
