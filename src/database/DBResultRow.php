...
class DBResultRow {
	...
	protected function getValue( $key ) {
		if ( !$this->hasKey( $key ) ) {
			throw new \wplibs\exception\DatabaseException( "Could not find key '$key' in actual result set" );
		}
	...
	protected function store( $forceOverwritePrimaryKeys = false ) {
		if ( $this->deleted ) {
			throw new \wplibs\exception\DatabaseException( "Could not store deleted object -> " . var_export( $this->row, true ) );
		}
