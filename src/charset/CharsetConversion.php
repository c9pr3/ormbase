...
class CharsetConversion {
	...
	static $conversionmap = [ 'ä' => 'ae', 'ü' => 'ue', 'ö' => 'oe', 'Ä' => 'Ae', 'Ü' => 'Ue', 'Ö' => 'Oe', 'ß' => 'ss', 'é' => 'e' ];
	...
	public static function checkInternalEncoding() {
		if ( mb_internal_encoding() != 'UTF-8' ) {
			throw new \wplibs\exception\CharsetConversionException( 'mb_internal_encoding: UTF-8 needed but is ' . mb_internal_encoding() );
		}

		if ( mb_regex_encoding() != 'UTF-8' ) {
			throw new \wplibs\exception\CharsetConversionException( 'mb_regex_encoding: UTF-8 needed but is ' . mb_regex_encoding() );
		}
	}
	...
	public static function unichr( $c ) {
		if ( $c <= 0x7F ) {
            ...
            throw new \wplibs\exception\CharsetConversionException( 'invalid char in unichr ' . intval( $c ) );
		}
	}
	...
	public static function toUTF8( $text, $useIconvIfExists = false ) {
		...
		if ( !is_string( $text ) && !is_null( $text ) ) {
			throw new \wplibs\exception\CharsetConversionException( "ToUTF8 expected string but got " . var_export( $text, true ) );
		}
		$text = trim( $text );
		$converted = '';
		$runcnt = 0;
		while ( $converted != $text ) {
			...
			if ( $runcnt > 10 ) {
				throw new \wplibs\exception\CharsetConversionException( 'loop detected during conversion' );
			}
			$runcnt++;
		}
	...
	public static function toUTF8Convert( $text, $useIconvIfExists = false ) {
		if ( $useIconvIfExists === true && function_exists( 'iconv' ) ) {
			...
			throw new \wplibs\exception\CharsetConversionException( 'toUTF8Convert: missing iconv' );
		} else {
	...
	public static function utf8ToIsoConvert( $text, $useIconvIfExists = false ) {
		if ( $useIconvIfExists === true && function_exists( 'iconv' ) ) {
			...
			throw new \wplibs\exception\CharsetConversionException( 'utf8ToIsoConvert: missing iconv' );
		} else {
	...
	protected static function checkUTF8( $in ) {
		if ( !mb_check_encoding( $in, 'UTF-8' ) ) {
			throw new \wplibs\exception\CharsetConversionException( 'checkUTF8: String is not valid UTF-8' );
		}
